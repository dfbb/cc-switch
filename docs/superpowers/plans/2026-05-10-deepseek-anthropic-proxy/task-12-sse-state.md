# T12: sse_state.rs — SSE 块策略状态机

> **并行：** ✅ 可与 T02–T11/T17 并行（无依赖）。

**Goal:** 实现 `SseBlockPolicyState` + `UpstreamBlockState` 结构体与核心转换函数 `transform_native_sse_block_event`（5 条规则）及 `should_drop_block_type`。
...
pub struct SseBlockPolicyState {
    ...
    pub by_upstream: std::collections::HashMap<usize, UpstreamBlockState>,
    pub dropped_indexes: std::collections::HashSet<usize>,
    pub pending_suppressed_stops: std::collections::HashSet<usize>,
...
pub struct UpstreamBlockState {
    ...
    pub open: bool,
    pub last_start_block: Option<serde_json::Value>,
}
...
mod tests_sse_state {
    ...
    fn make_event(event_type: &str, data: serde_json::Value) -> String {
        format!("event: {}\ndata: {}", event_type, data.to_string())
    }
    ...
    fn test_message_start_rewrites_model() {
        let mut state = make_state();
        let event = make_event("message_start", json!({
            "type": "message_start",
        ...
        let parsed: serde_json::Value = serde_json::from_str(
            result[0].strip_prefix("event: message_start\ndata: ").unwrap()
        ).unwrap();
    ...
    fn test_content_block_start_text_assigned_index() {
        let mut state = make_state();
        let event = make_event("content_block_start", json!({
        ...
        assert_eq!(result.len(), 1);
        let data_str = result[0].lines()
            .find(|l| l.starts_with("data:"))
            .unwrap()
            .strip_prefix("data: ").unwrap();
    ...
    fn test_thinking_block_kept_when_enabled() {
        ...
        let result = transform_native_sse_block_event(&event, &mut state, "fake", true);
        // thinking enabled: block should be forwarded (possibly with index rewrite)
    ...
    fn test_content_block_delta_dropped_for_dropped_index() {
        ...
        let event = make_event("content_block_delta", json!({
    ...
    fn test_content_block_delta_remaps_index() {
        let mut state = make_state();
        // First open block at upstream 0 → down 0
        {
            let event = make_event("content_block_start", json!({
                "type": "content_block_start",
                "index": 0,
        ...
        let data_str = result[0].lines()
            ...
            .strip_prefix("data: ").unwrap();
        let parsed: serde_json::Value = serde_json::from_str(data_str).unwrap();
        assert_eq!(parsed["index"], 0); // mapped downstream index
    }
    ...
    fn test_content_block_stop_for_dropped_is_empty() {
        ...
        let event = make_event("content_block_stop", json!({
    ...
    fn test_two_blocks_open_second_synthesizes_stop_for_first() {
        ...
        // Open block 0 (text)
        {
            let e = make_event("content_block_start", json!({
                "type": "content_block_start",
                "index": 0,
                "content_block": {"type": "text"}
            }));
        ...
        // Open block 1 (text) — should synthesize stop for block 0 first
        let e2 = make_event("content_block_start", json!({
            "type": "content_block_start",
            "index": 1,
            "content_block": {"type": "text"}
        }));
        let result = transform_native_sse_block_event(&e2, &mut state, "fake", true);
        // Should have [synthetic_stop, new_start] = 2 elements
    ...
    fn test_index_remapping_is_sequential() {
        let mut state = make_state();
        // Drop block 0 (thinking disabled)
        {
            let e = make_event("content_block_start", json!({
                "type": "content_block_start",
                "index": 0,
        ...
        // Block 1 (text) should get downstream index 0
        let e = make_event("content_block_start", json!({
            "type": "content_block_start",
            "index": 1,
            "content_block": {"type": "text"}
        ...
        assert_eq!(result.len(), 1);
        let data_str = result[0].lines()
            .find(|l| l.starts_with("data:")).unwrap()
            .strip_prefix("data: ").unwrap();
        let parsed: serde_json::Value = serde_json::from_str(data_str).unwrap();
...
```bash
cd src-tauri && cargo test deepseek_anthropic::sse_state::tests_sse_state 2>&1 | tail -15
```
...
pub fn transform_native_sse_block_event(
    ...
    let (event_type, mut payload) = match parse_event(event) {
        Some(v) => v,
        None => {
            log::warn!("malformed event line: {}", event);
            state.bypass = true;
            return vec![event.to_string()];
        }
    };
    ...
    match event_type {
        "message_start" => {
            if let Some(model) = payload.get_mut("message")
                .and_then(|m| m.as_object_mut())
                .and_then(|m| m.get_mut("model"))
            {
                *model = Value::String(fake_model.to_string());
            } else {
                log::warn!("model field missing in message_start");
                return vec![event.to_string()];
            }
        ...
        "content_block_start" => {
            let up_index = match payload.get("index").and_then(|v| v.as_u64()) {
                Some(i) => i as usize,
                None => {
                    log::warn!("missing index/type in content_block_start");
                    state.bypass = true;
                    return vec![event.to_string()];
                }
            };
            let block_type = match payload.get("content_block")
            ...
            let open_ups: Vec<usize> = state.by_upstream.iter()
                .filter(|(_, b)| b.open)
                .map(|(k, _)| *k)
                .collect();
            for open_up in open_ups {
                ...
                let stop = json!({"type":"content_block_stop","index": bs.down_index});
            ...
            if let Some(obj) = start_payload.as_object_mut() {
                obj.insert("index".into(), Value::Number(down_index.into()));
            }

            state.by_upstream.insert(up_index, UpstreamBlockState {
                block_type,
                down_index,
                open: true,
        ...
        "content_block_delta" => {
            let up_index = match payload.get("index").and_then(|v| v.as_u64()) {
                Some(i) => i as usize,
                None => {
                    log::warn!("missing index in content_block_delta");
                    state.bypass = true;
                    return vec![event.to_string()];
                }
            };
            ...
            // Check if delta type should be dropped
            if let Some(delta_type) = payload.get("delta")
            ...
            let down_index = if let Some(bs) = state.by_upstream.get_mut(&up_index) {
                if !bs.open {
                    // Closed but delta arrived: reopen with new index
                    ...
                    // Synthesize new start
                    let start_data = if let Some(mut s) = last_start {
                        if let Some(obj) = s.as_object_mut() {
                            obj.insert("index".into(), Value::Number(new_down.into()));
                        }
                        s
                    } else {
        ...
        "content_block_stop" => {
            let up_index = match payload.get("index").and_then(|v| v.as_u64()) {
                Some(i) => i as usize,
                None => {
                    log::warn!("missing index in content_block_stop");
                    state.bypass = true;
                    return vec![event.to_string()];
                }
            };
...
```bash
cd src-tauri && cargo test deepseek_anthropic::sse_state::tests_sse_state 2>&1 | tail -10
```
...
```bash
git add src-tauri/src/proxy/providers/deepseek_anthropic/sse_state.rs
git commit -m "feat(deepseek): implement sse_state block policy state machine"
```
